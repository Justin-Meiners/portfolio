interface Env {
	...
	SPOTIFY_SONG_OF_MONTH_PLAYLIST_ID: string
}
...
export default {
	async fetch(request: Request, env: Env): Promise<Response> {
		...
		try {
			...
			const [playback, artists, tracks, recent, songOfMonth] = await Promise.all([
				...
				spotifyRequest<SpotifyPlaylistItems>(
					`/playlists/${env.SPOTIFY_SONG_OF_MONTH_PLAYLIST_ID}/items?limit=50`,
					accessToken,
				).catch((error) => {
					console.error('Song of the month request failed', error)
			...
			const metrics = {
				...
				songOfMonth: songOfMonth.items
					.flatMap((entry) => {
						if (!entry.item || !entry.added_at) return []
